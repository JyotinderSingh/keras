...
class Int4ProjectionHandlers:
    """`_build_projection` / `_qvariable_projection` / `_call_projection` /
    `_encode_projection` / `_quantize_projection`.
    ...
    forward pass is one path: dequantize through the mode's `QVariable`
    ...
    def _view(self, layer, geometry, codes, scale, zero_point, g_idx):
        """The `QVariable` over given tensors (the layer's, or traced ones)."""
