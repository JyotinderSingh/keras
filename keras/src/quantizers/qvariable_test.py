...
from keras.src.quantizers.qvariable import TernaryTrits
...
class WeightSchemeTest(testing.TestCase):
    def test_fields_and_derived_properties(self):
        ...
        self.assertTrue(scheme.has_zero_point)
        self.assertTrue(scheme.grouped)
        self.assertIn("group_size=32", repr(scheme))

        unsigned = WeightScheme(code_range=(0, 15))
        self.assertEqual(unsigned.bits, 4)
        self.assertFalse(unsigned.signed)
        self.assertFalse(unsigned.has_zero_point)
        self.assertFalse(unsigned.grouped)
        # The width is the smallest that holds every code.
        self.assertEqual(WeightScheme(code_range=(-127, 127)).bits, 8)
        self.assertEqual(WeightScheme(code_range=(0, 7)).bits, 3)
        self.assertEqual(WeightScheme(code_range=(-1, 1)).bits, 2)
...
class QVariableTest(testing.TestCase):
    def test_validates_variables_against_scheme(self):
        ...
        # `align_scale` lays the scale out itself, so the scheme must not
        # also name an axis.
    ...
    def test_per_channel_scale_broadcasts_along_its_axis(self):
        ...
        view = QVariable(
            ...
            scale=scale,
            layout=NoPack(),
            scheme=_int8_scheme(0),
            shape=(2, 2),
        )
        self.assertAllClose(view.dequantize(), [[1.0, 2.0], [1.5, 2.0]])

    def test_per_channel_scale_on_a_middle_axis(self):
        codes = np.arange(12, dtype="int8").reshape(2, 3, 2)
        scale = np.array([1.0, 2.0, 4.0], "float32")
        view = QVariable(
            codes=codes,
            scale=scale,
            layout=NoPack(),
            scheme=_int8_scheme(1),
            shape=(2, 3, 2),
        )
        self.assertAllClose(view.dequantize(), codes / scale[None, :, None])
    ...
    def test_align_scale_lays_out_a_stored_scale(self):
        ...
        stored = np.array([[2.0], [4.0], [6.0]], "float32")
        view = QVariable(
            codes=codes,
            scale=stored,
            layout=NoPack(),
            scheme=_int8_scheme(None),
            shape=(2, 3),
            align_scale=ops.transpose,
        )
        self.assertAllClose(
            view.dequantize(), [[1.0, 1.0, 1.0], [4.0, 2.5, 2.0]]
        )
    ...
    def test_scalar_scale(self):
        ...
        view = QVariable(
            ...
            layout=NoPack(),
            scheme=WeightScheme(code_range=(-1, 1)),
            shape=(1, 3),
        )
    ...
    def test_shape_restores_an_nd_kernel(self):
...
class LayerViewTest(testing.TestCase):
    ...
    def test_dense_unpack_backs_the_kernel_property(self, mode, config):
        ...
        self.assertEqual(view.shape, (7, 5))
        self.assertAllClose(view.unpack(), layer.kernel)
        if mode == "int4":
            expected = packing.unpack_int4(layer._kernel, layer.units, axis=-1)
    ...
    def test_dense_dequantize_matches_the_stored_scales(self):
        ...
        view = layer._qvariable()
        unpacked = packing.unpack_int4(layer._kernel, layer.units, axis=-1)
        expected = quantizers.dequantize_grouped(
    ...
    def test_dequantize_reconstructs_the_float_kernel(self):
        ...
        self.assertAllClose(
            layer._qvariable().dequantize(), reference, atol=1e-2
        )
    ...
    def test_encode_is_what_quantize_stores(self, mode, config):
        ...
        if zero is None:
            self.assertIsNone(layer._qvariable().zero_point)
        else:
    ...
    def test_einsum_dense_views(self):
        ...
        view = layer._qvariable()
        self.assertEqual(view.shape, (6, 2, 3))
        self.assertAllClose(view.unpack(), layer.kernel)
        self.assertEqual(tuple(view.dequantize().shape), (6, 2, 3))

    def test_int8_projection_scale_layout(self):
        # A matmul kernel's scale runs along its output axis; an einsum
        # kernel's is stored for the outputs and the geometry lays it out.
        view = self._dense("int8")._qvariable()
        self.assertEqual(view.scheme.channel_axis, -1)
        ...
        layer.quantize("int8")
        view = layer._qvariable()
        self.assertIsNone(view.scheme.channel_axis)
        self.assertIsNotNone(view.align_scale)
        self.assertEqual(view.shape, (6, 2, 3))
        self.assertAllClose(view.dequantize(), reference, atol=1e-2)
    ...
    def test_embedding_views(self):
        ...
        layer.quantize("int4", config=Int4QuantizationConfig(block_size=2))
        view = layer._qvariable()
        self.assertEqual(view.shape, (9, 4))
        self.assertAllClose(view.unpack(), layer.embeddings)
        unpacked = packing.unpack_int4(
    ...
    def test_modes_without_codes_have_no_view(self):
        ...
        layer.build((None, 4))
        self.assertIsNone(layer._qvariable())
