...
- the mode's math: the `build`/`call`/`quantize` methods create the mode's
  ...
  `encode` turns a float weight into the stored form and `qvariable` exposes
  the stored variables as a `QVariable` view (`keras.src.quantizers.qvariable`)
...
class QuantizationStrategy:
    ...
    def qvariable(self, layer):
        """Returns the `QVariable` of `layer`'s quantized weight, or `None`.
    ...
    def qvariables(self, layer):
        """All `QVariable` views of `layer`'s quantized weights.
