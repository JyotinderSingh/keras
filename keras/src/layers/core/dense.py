...
class Dense(Layer):
    ...
    def kernel(self):
        ...
        # kernel's own shape by the mode's `QVariable` view; otherwise the
        ...
        kernel = self._kernel if qvariable is None else qvariable.unpack()
    ...
    def _get_kernel_with_merged_lora(self):
        ...
        lora_kernel_b`, scaled) is folded into the stored weight by the
        mode (`QuantizationStrategy.merge_lora_delta`): int8, int4 and
        ternary dequantize, add the delta and encode the sum with a fresh
        scale; GPTQ and AWQ keep their calibrated scale, zero point, group
        index and AWQ scales and round the merged weight onto that grid.
