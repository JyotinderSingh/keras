...
class DenseTest(testing.TestCase):
    ...
    def test_int4_quantization_block_size(self, block_size):
        ...
        self.assertEqual(
            layer._qvariable().scheme.group_size,
    ...
    def test_dense_ternary_lora_merge_on_save(self):
        ...
        merged = ops.convert_to_numpy(qvariable.dequantize()) + (
            layer.lora_alpha / layer.lora_rank
        ) * (
            ops.convert_to_numpy(layer.lora_kernel_a)
            @ ops.convert_to_numpy(layer.lora_kernel_b)
        )
        codes, scale = ternarize(merged)
    ...
    def test_ternary_mode_encode(self):
        ...
        codes, scale, zero = strategy_registry.get_strategy("ternary").encode(
            layer, weight
        )
        self.assertIsNone(zero)
        expected_codes, expected_scale = ternarize(weight)
        self.assertAllClose(
            quantizers.unpack_ternary(codes, 8, axis=-1), expected_codes
        )
        # `encode` returns the stored divisor, `1 / beta`, as `ternarize` does.
        self.assertAllClose(scale, expected_scale)
    ...
    def test_int4_string_matches_default_config(self):
        ...
        self.assertEqual(layer_str._qvariable().scheme.group_size, 128)
