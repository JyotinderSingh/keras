...
class TernaryDense(Layer):
    ...
    **Inference note.** `quantize("ternary")` is a storage format, not a
    ...
    def kernel(self):
        ...
        # `QVariable` view; otherwise the float kernel.
        qvariable = self._qvariable()
        return self._kernel if qvariable is None else qvariable.unpack()
...
class _TernaryDenseGeometry(ProjectionGeometry):
    ...
    def ternary_values(self):
        layer = self.layer
        # The rule of the straight-through kernel used in training, evaluated
        # under the layer's autocast scope so the frozen codes are exactly
        # the trits a mixed-precision forward pass used; the scale is the
